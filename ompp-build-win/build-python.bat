@echo off
REM build openM++ Python examples

setlocal enabledelayedexpansion

REM push into ompp root and make log directory if not exist

if not exist ompp (
  @echo ERROR: missing source code directory: ompp
  EXIT 1
)

pushd ompp

if not exist log mkdir log

REM log build environment 

@echo %DATE% %TIME% Build openM++ Python examples
@echo Log file: log\build-python.log
@echo %DATE% %TIME% Build openM++ Python examples > log\build-python.log

REM get source code from git, if directory not already exist

if not exist ompp-python (
  
  call :do_cmd_line_log log\build-python.log "git clone https://github.com/openmpp/python ompp-python"
  
) else (
  @echo Skip: git clone
  @echo Skip: git clone >> log\build-python.log
)

@echo %DATE% %TIME% Done.
@echo %DATE% %TIME% Done. >> log\build-python.log

popd
goto :eof

REM end of main body

REM helper subroutine to execute command, log it and check errorlevel
REM arguments:
REM  1 = log file path
REM  2 = command line

:do_cmd_line_log

set c_log=%1
set c_line=%~2

@echo %c_line%
@echo %c_line% >> %c_log%

%c_line% >> %c_log% 2>&1
if ERRORLEVEL 1 (
  @echo FAILED.
  @echo FAILED. >> %c_log%
  EXIT
) 
exit /b

