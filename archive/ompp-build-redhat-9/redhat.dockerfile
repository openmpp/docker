# Docker image to build openM++ for RedHat (Rocky Linux) 9
#
# Examples of build and arguments default values:
#   docker build -t openmpp/openmpp-build:redhat-9 -f redhat.dockerfile .
#   docker build -t openmpp/openmpp-build:redhat-9 -f redhat.dockerfile --build-arg OMPP_USER=ompp .
#   docker build -t openmpp/openmpp-build:redhat-9 -f redhat.dockerfile --build-arg OMPP_GROUP=ompp .
#   docker build -t openmpp/openmpp-build:redhat-9 -f redhat.dockerfile --build-arg OMPP_UID=1999 .
#   docker build -t openmpp/openmpp-build:redhat-9 -f redhat.dockerfile --build-arg OMPP_GID=1999 .
#
...
#     ./build-all
#
#   docker run .... -e OM_MSG_USE=MPI      openmpp/openmpp-build:redhat-9 ./build-openm
#   docker run .... -e MODEL_DIRS=modelOne openmpp/openmpp-build:redhat-9 ./build-models
#   docker run .... openmpp/openmpp-build:redhat-9 ./build-go
#   docker run .... openmpp/openmpp-build:redhat-9 ./build-r
#   docker run .... openmpp/openmpp-build:redhat-9 ./build-ui
#   docker run .... openmpp/openmpp-build:redhat-9 ./build-tar-gz
#
#   docker run -it openmpp/openmpp-build:redhat-9 bash
...
# describe image
#
LABEL name=openmpp/openmpp-build:redhat-9
LABEL os=Linux
LABEL license=MIT
