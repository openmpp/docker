...
To run openM++ model do:
  ...
  docker run .... openmpp/openmpp-run:debian-10 ./MyModel
  
Examples:
  docker run \
    -v $HOME/models:/home/models \
    -e OMPP_USER=models -e OMPP_GROUP=models -e OMPP_UID=$UID -e OMPP_GID=`id -g` \
    openmpp/openmpp-run:debian-10 \
    ./MyModel
...
To start shell do:
  ...
  docker run -it openmpp/openmpp-run:debian-10 bash

  docker run \
    -v $HOME:/home/${USER} \
    -e OMPP_USER=${USER} -e OMPP_GROUP=`id -gn` -e OMPP_UID=$UID -e OMPP_GID=`id -g` \
    -it openmpp/openmpp-run:debian-10 bash
