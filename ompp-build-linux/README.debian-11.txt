...
To build openM++ do:
  ...
  docker run .... openmpp/openmpp-build:debian-11 ./build-all
...
Examples:
  ...
  docker run .... -e MODEL_DIRS=RiskPaths,IDMM      openmpp/openmpp-build:debian-11 ./build-all
  docker run .... -e OM_BUILD_CONFIGS=RELEASE,DEBUG openmpp/openmpp-build:debian-11 ./build-all
  docker run .... -e OM_MSG_USE=MPI                 openmpp/openmpp-build:debian-11 ./build-all
...
To build openM++ libraries and omc compiler do:
  ...
  docker run .... openmpp/openmpp-build:debian-11 ./build-openm
  
  Environment variables to control "build-openm": OM_BUILD_CONFIGS, OM_MSG_USE, OMPP_BUILD_TAG, OMPP_CPP_BUILD_TAG

To build models do:

  docker run .... openmpp/openmpp-build:debian-11 ./build-models
...
To build openM++ tools do any of:
  ...
  docker run .... openmpp/openmpp-build:debian-11 ./build-go # Go oms web-service and dbcopy utility
  docker run .... openmpp/openmpp-build:debian-11 ./build-r  # openMpp R package
  docker run .... openmpp/openmpp-build:debian-11 ./build-ui # openM++ UI
  
To create openmpp_debian-11_YYYYMMDD.tar.gz archive:
